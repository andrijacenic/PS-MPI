...
komunikatora koji obuhvata sve procese dostaviti ove vrednosti i odštampati ih. Ilustrovati
raspored procesa i program testirati za različite dimenzije Cartesian strukture.
*/
#include <mpi.h>
#include <stdio.h>
#include <math.h>
...
int main(int argc, char* argv[]) {
	...
	MPI_Comm_split(cartComm, coords[0] > coords[1], rank, &trougaona);
	MPI_Comm_rank(trougaona, &rankTrougaona);
	printf("troug : %d, rank : %3d, t rank %3d coords : (%d, %d)\n", coords[0] > coords[1], rank, rankTrougaona, coords[0], coords[1]);
